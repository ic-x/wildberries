    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Text("Отправили код на номер \(viewModel.verificationData.phoneNumber)")
                ...
                HStack(spacing: 10) {
                    ForEach(0..<4, id: \.self) { index in
                        let digit = inputCode.count > index ? String(inputCode[inputCode.index(inputCode.startIndex, offsetBy: index)]) : ""
                ...
                TextField("", text: $inputCode)
                    ...
                    .onChange(of: inputCode) { oldValue, newValue in
                        viewModel.validateCode(newValue)
                    }
            ...
            .onChange(of: viewModel.isCodeCorrect) { oldValue, newValue in
                if newValue {
                    navigationPath.append(NavigationItem.profileAccountView(phoneNumber: viewModel.verificationData.phoneNumber))
